...
//          Response.Status.NO_CONTENT);
  ...
  public void aGetSubscriptionsRequestIsMadeForCluster(String clusterName) throws Throwable {
    ...
    params.put("clusterName", clusterName);
    callSubscription("GET", Optional.of(params), Optional.absent(), Response.Status.OK);
  }
  ...
  public void aSubscriptionIsCreatedForCluster(String clusterName) throws Throwable {
    HashMap<String, String> params = new HashMap<>();
    params.put("clusterName", clusterName);
    callSubscription("POST", Optional.of(params), Optional.absent(), Response.Status.OK);
  }
  ...
  public void aGetSubscriptionRequestIsMadeForTheLastInsertedID() throws Throwable {
    ...
    DiagEventSubscription lastCreated = TestContext.LAST_CREATED_EVENT_SUBSCRIPTIONS.get(
            TestContext.LAST_CREATED_EVENT_SUBSCRIPTIONS.size() - 1);
    callSubscription("GET", Optional.absent(), Optional.of(lastCreated.getId()), Response.Status.OK);
  }
  ...
  public void theLastCreatedSubscriptionIsDeleted() throws Throwable {
    synchronized (BasicSteps.class) {
      ...
      ReaperTestJettyRunner runner = RUNNERS.get(RAND.nextInt(RUNNERS.size()));
      Response response = runner.callReaper("DELETE",
              "/diag_event/subscription/" + lastCreated.getId(), Optional.absent());
  ...
  public void theFollowingSubscriptionsExist(List<Map<String, String>> subscriptions) throws Throwable {
    synchronized (BasicSteps.class) {
      ...
      for (Map<String, String> sub : subscriptions) {
        Response response = runner.callReaper(
                "POST", "/diag_event/subscription", Optional.of(sub));
        Assertions.assertThat(response.getStatus()).isEqualTo(Response.Status.OK.getStatusCode());
  ...
  public void theReturnedListOfSubscriptionsIs(List<Map<String, String>> subscriptions) throws Throwable {
    ...
    List<DiagEventSubscription> expected = subscriptions.stream().map(DiagEventSubscriptionMapper::fromParamMap)
            .collect(Collectors.toList());
    Assertions.assertThat(TestContext.LAST_RETRIEVED_EVENT_SUBSCRIPTIONS.size()).isEqualTo(expected.size());
    List<DiagEventSubscription> lastRetrieved = TestContext.LAST_RETRIEVED_EVENT_SUBSCRIPTIONS.stream()
            .map(s -> s.withId(null)).collect(Collectors.toList());
    for (DiagEventSubscription sub : expected) {
      Assertions.assertThat(lastRetrieved).contains(sub);
    }
  }

  private void callSubscription(String method, Optional<Map<String, String>> params, Optional<UUID> id,
                                Response.Status... expectedStatuses) {
    synchronized (BasicSteps.class) {
      ReaperTestJettyRunner runner = RUNNERS.get(RAND.nextInt(RUNNERS.size()));
      ...
      if (id.isPresent()) {
        ...
        TestContext.LAST_RETRIEVED_EVENT_SUBSCRIPTIONS.add(SimpleReaperClient
                .parseEventSubscriptionJSON(responseData));
      } else {
        TestContext.LAST_RETRIEVED_EVENT_SUBSCRIPTIONS = SimpleReaperClient
                .parseEventSubscriptionsListJSON(responseData);
      }
    }
  }
}
